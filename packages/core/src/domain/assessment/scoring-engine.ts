import { ARCHETYPES, DIMENSIONS } from '../archetypes/archetypes.data';
...
/** Un contador a cero por arquetipo, derivado de los datos. */
function registroEnCero(): Record<ArchetypeId, number> {
  return Object.fromEntries(
    (Object.keys(ARCHETYPES) as ArchetypeId[]).map(id => [id, 0])
...
export function calculateAssessmentResult(
  ...
  // Los registros salen de los datos, no de una lista escrita a mano: enumerar
  // los arquetipos aqui es lo que dejo este motor puntuando doce cuando el
  // dominio ya tenia dieciocho, sin que nada fallara.
  const rawScores = registroEnCero();
  const maxPossibleScores = registroEnCero();
  ...
  const normalizedScores = registroEnCero();
  ...
  // Cada dimension promedia a los arquetipos que la habitan segun DIMENSIONS.
  // Sumarlos a mano obliga a acordarse de dos sitios cada vez que entra uno nuevo.
  const dimensionScores = Object.fromEntries(
    (Object.keys(DIMENSIONS) as DimensionId[]).map(d => {
      const habitantes = DIMENSIONS[d].archetypes;
      const suma = habitantes.reduce((acc, id) => acc + normalizedScores[id], 0);
      return [d, habitantes.length ? Math.round(suma / habitantes.length) : 0];
    })
